...
//   const calculateAllAmounts = (details: InvoiceDetail[]) => {
//       if (!isInitialized.current) return;
...
//           ?.find((r) => r.id == d.productID)?.units?.find((r) => r.id == d.qtyUnitID)?.conversionRate;
//         console.log('changes in the price:' + d.quantity);
//         let qty = value ? parseFloat(value) : 0;
//         let amt = qty * d.price * (convRate ?? 0);
//         let disc = (amt * d.discPercent) / 100;
//         let netAmt = amt - disc;
...
//       const updatedDetails = prev.details.map((d, i) => {
//         console.log('changes in the price:' + d.price);
...
//   interface Config {
//     label: string;
//     render: (salesInvoiceDetails: InvoiceDetail, index: number) => React.ReactNode;
//   }
...
//       },
//     },
//     { label: 'Gross Amount', render: (p) => p.amount.toFixed(2) },
